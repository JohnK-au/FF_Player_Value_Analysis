...
| 2026-06-28 | Position component activated — 4-sub-metric composite (Slot Count + Marginal Gap + Total Impact + Supply-Demand), equal weights, min-max [0, 100]. RB=100, WR=35.2, TE=21.2, QB=0 | Phase 4.5: positional importance for team success (NOT cap pricing — that's Phase 5+). In a 1-QB league, QB has smallest elite-vs-replacement gap (~8 OFV) and only 1 slot/team, so total positional advantage is smallest. RB combines biggest gap (~19) with 2.5 slots → highest total impact. Position at 5% combine weight → max ±2.5 dynasty-value swing per player. Equal weights are v1; tune after digesting impact. See [position.md](position.md). |
| 2026-06-28 | **Position component v2 — VORP-Deep Total Impact (PPG-based, T-only).** RB=100, WR=93.1, TE=8.6, QB=0 | Phase 4.5 iteration: v1's OFV-based M was within-position normalized, hiding cross-position absolute PPG differences. v2 fixes by using absolute 2025 PPG with VORP-Deep replacement (avg PPG of ranks 3N+1 to 4N — the "deep FA" tier). Per user's "16-24 range for QBs" framing, scaled to other positions. T-only weighting (T = M × S). Result: WR climbs to near-tie with RB (deep-FA T = 32.3 vs 33.7 PPG/week), correctly reflecting user's intuition that "Chase-to-next-tier-WR gap is bigger than Kelce-to-next-tier-TE in absolute PPG." TE stays low; multi-tier (elite / startable / bench / reserve) concentration refinement deferred. See [position.md](position.md). |
| 2026-06-28 | **Phase 5 combine workshop — `ofv_weighted_sum` with current defaults reaffirmed.** OFV 0.55 + Age 0.20 + Injury 0.15 + Position 0.05 + Intangibles 0.05. | Five candidate combine variants generated (see [`src/viz/combine_variants.py`](../../src/viz/combine_variants.py)): v1 current, v2 OFV+Age boost (0.65/0.30/0.10), v3 hybrid (weighted base × multiplicative overlays), v4 full multiplicative, v5 Position-boosted (Position to 0.10). User reviewed top 5 cross-position rankings per variant and chose v1 current. Reasoning: cleanest math properties (weights sum to 1; range stays in [0, 100]; no multiplicative amplification effects). Multiplicative + hybrid variants reshuffled WR ordering at the top and v2 promoted Drake Maye into top 5 (by dropping Position penalty + boosting Age), but neither change felt like an improvement. Workshop viz kept in [`src/viz/combine_variants.py`](../../src/viz/combine_variants.py) for future re-runs after data changes or weight re-tuning. |
| 2026-07-03 | **Phase 5.5 pricing engine locked (`C_user_targets`).** Multi-stage cap-unit pricing pipeline layered ON TOP of V2 (V2 master CSV never modified). Params: α=1.25, baselines QB=41/RB=29/WR=34/TE=31, pool_scale=1.45, age_band=[0.85, 1.15]. | New module [`src/models/pricing.py`](../../src/models/pricing.py) + workshop viz [`src/viz/pricing_variants.py`](../../src/viz/pricing_variants.py). Pipeline: per-position replacement baseline → above_baseline_dv → non-linear scarcity (^α) → rate × age multiplier × multi-year age decay. Locked via 3 iterations of workshop feedback: (1) linear sweep identified compression, (2) baseline_offset sweep with α=1.2 dropped elites to $165 but user wanted higher, (3) baseline_override + pool_scale=1.45 hits user's stated targets (Gibbs $240, Chase $179, McBride $175, Maye $134). Output written to `data/processed/player_pricing_2026.csv` (joins to V2 master on espn_id). Sign convention matches V1 app: positive surplus = overpaid (red). See [pricing.md](pricing.md). |
